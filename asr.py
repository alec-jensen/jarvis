            ...
            while True:
                ...
                if not is_active_listening:
                    if rms > SPEAKING_THRESHOLD:
                        print(f"Speech detected, rms: {rms:.5f}")
                        if on_speech_start:
                            on_speech_start()
...
if __name__ == "__main__":
    ...
    print(f"Current SPEAKING_THRESHOLD: {SPEAKING_THRESHOLD}")
    print("Live RMS values will be printed. Speak to see your RMS and tune the threshold.")

    class DebugAudioInput(AudioInput):
        def listen(self, on_speech_start=None, on_speech_end=None):
            print("AudioInput: Ready. Speak to activate.")
            audio_buffer = []
            listening_start_time = None
